...
func DbOpen(driver string, dsn string, showLog bool, cacherOpen bool, cacherMaxSize int, cacherExpired string) (d *DbModel, err error) {
	...
	if cacherOpen {
		expired, _ := time.ParseDuration(cacherExpired)
		cacher := xorm.NewLRUCacher2(xorm.NewMemoryStore(), expired, cacherMaxSize)
		d.Engine.SetDefaultCacher(cacher)
	}

	if showLog {
		d.Engine.ShowExecTime(true)
...
func (d *DbModel) Close() error {
	return d.Engine.Close()
}

func (d *DbModel) MFilter(s *xorm.Session, search map[string]interface{}, filters map[string]string) {
...
func (d *DbModel) MSfilter(search interface{}, filters map[string]string, isand bool) builder.Cond {
...
func (d *DbModel) MFind(beans interface{}, bean interface{}, s map[string]interface{}, f map[string]string, after func(*xorm.Session, map[string]interface{})) (count int64, page int, limit int, err error) {
	if s == nil {
...
func (d *DbModel) MFirst(bean interface{}, w map[string]interface{}, f map[string]string) (bool, error) {
	session := d.Engine.NewSession()
	defer session.Close()
